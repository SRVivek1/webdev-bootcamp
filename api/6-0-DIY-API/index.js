...
// middle ware to log app req details
...
app.get("/joke/:id", (req, res) => {
  const joke = jokes.find((j) => {
    if (j.id == req.params.id) return true;
  });
...
app.get("/joke", (req, res) => {
  ...
  const result = jokes.filter(
    (j) => j.jokeType.toLowerCase() == req.query.type.toLowerCase()
  );
  console.log(result);
...
app.post("/jokes", (req, res) => {
  ...
  jokes.push(joke);
  res.json(jokes.find((j) => j.id === joke.id));
});
...
app.put("/joke/:id", (req, res) => {
  ...
  const jokeIndex = jokes.findIndex((e) => e.id === id);
  jokes[jokeIndex] = newJoke;

  const result = jokes.find((e) => id === e.id);
  console.log(result);
...
app.patch("/joke/:id", (req, res) => {
  ...
  console.log(`Old Joke : ${JSON.stringify(oldJoke)}`);
  console.log(`Updated Joke : ${JSON.stringify(newJoke)}`);
  console.log(`Joke index: ${jokeIndex}`);

  jokes[jokeIndex] = newJoke;

  res.json(jokes.find((e) => e.id === id));
});
...
app.delete("/joke/:id", (req, res) => {
  ...
  const i = jokes.findIndex((e) => e.id === id);
  if (i > -1) {
    const deleted = jokes.splice(i, 1);
    res.status(200).json(deleted);
  } else {
    res
      .status(404)
      .json({ error: `Jokes with ${id} not found. No jokes were deleted.` });
  }
  res.json(t);
});
