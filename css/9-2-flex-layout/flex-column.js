// default flex direction is set to row
const dmsg =
  "By Default flex-direction is set to row, change the value from menu to see the impact.";
updatePropDesc(dmsg);
...
flexDirection.addEventListener("change", function () {
  ...
  const msg =
    flexDirection.value === "column"
      ? flexDirection.value + ": show flex items from top to bottom."
      : flexDirection.value + ": show flex items from bottom to top.";
  updatePropDesc(msg);
});
...
// Container height
const cmhElements = document.getElementsByClassName("container");
// Ensure at least one element exists
if (cmhElements.length > 0) {
  const cmh = cmhElements[0].offsetHeight; // Access the first element's height
  document.getElementById("container-height-input").value = cmh;
}

//update container width
const chi = document.getElementById("container-height-input");
chi.addEventListener("change", function () {
  cmhElements[0].style.height = chi.value + "px";
});
...
flexBasis.addEventListener("change", function () {
  ...
  for( let i = 0; i < t.length; i++) {
    t[i].style.flexBasis = flexBasis.value + "px";
  }
...
const flexOrderChange = () => {
  ...
  let msg =
    "Defines the order of flex items inside the container. Lower value appears first, default is 0.";
  updatePropDesc(msg);
};
flexOrder.addEventListener("change", flexOrderChange);

// align-self (Overrides align-items styling for Individual Items) - Allows an individual item to have a different vertical alignment.
const flexAlignSelf = document.getElementById("align-self");
const flexAlignSelfFunction = () => {
  let t = document.getElementsByClassName("box");
  // override for all items
  for (let i = 0; i < t.length; i++) {
    t[i].style.alignSelf = flexAlignSelf.value;
  }

  let msg =
    "Overrides align-items from the container. Works on the cross-axis (vertical in flex-direction: row). Default: auto (inherits from align-items of the parent).";
